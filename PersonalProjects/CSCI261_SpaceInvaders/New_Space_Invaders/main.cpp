...
int main() {
    ...
    vector <vector<Alien>> AlienVector(NUM_ALIENS_DOWN, vector<Alien>(NUM_ALIENS_ACROSS));

    for (int i = 0; i < NUM_ALIENS_DOWN; ++i) {
        for (int j = 0; j < NUM_ALIENS_ACROSS; ++j) {
            Alien tempAlien(texture, IntRect(32, 25, 100, 70),50 + 50 * j, 100 + 50 * i );
            AlienVector.at(i).at(j) =  tempAlien;
        }
    }
    ...
    RenderWindow window(VideoMode(640,640), "Space Invaders");
